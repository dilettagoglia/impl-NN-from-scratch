...
class Training:
    ...
    def __init__(self, net, loss, metr, lr, momentum, reg_type, lambd, lr_decay = None, limit_step = None, decay_rate = None, decay_steps = None):
        # TODO metric and loss name are patches to run the code. To modify
        self._net = net
        self._loss_name = loss
        self._loss = ErrorFunction.init_error_function(loss) # tuple composed by (error_function, error_function_der)
        ...
        self.lambd = lambd
        self.reg = Regularizations.init_regularization(reg_type) # tuple composed by (reg_function, reg_function_der)
        ...
        for epoch in tqdm.tqdm(range(epochs), desc="Iterating over epochs", disable=disable_tqdm):
            ...
            for batch_index in range(math.ceil(len(tr_x) / batch_size)):
                ...
                for pattern, target in zip(train_batch, targets_batch):
                    ...
                    epoch_tr_error = np.add(epoch_tr_error, self.loss[0](prediction=net_outputs, target=target))
                    ...
                    dErr_dOut = self.loss[1](prediction=net_outputs, target=target)
                ...
                for layer_index in range(len(net.layers)):
                    ...
                    net.layers[layer_index].weights = np.subtract(
                        ...
                        self.reg[1](w=net.layers[layer_index].weights, lambda_=self.lambd),
                    )
            ...
            # validation
            if val_x is not None:
                epoch_val_error, epoch_val_metric = net.evaluate(inp=val_x, targets=val_y, metr=self._metric_name, loss=self._loss_name)
